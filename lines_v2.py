...
def poss_partitions_special_v2(total_adj_lines,crossed_lines):
  # total_adj_lines is just the number of lines
  # crossed_lines is the number of lines we're intending to cross
  if total_adj_lines < crossed_lines:
    return -1  # since you can't cross off more lines than you have
...
def remove_4s_and_1_pairs(partition):
...
def poss_partitions_v2(partition,max_adj_lines):
  ...
  for adj_lines in range(1, max_adj_lines + 1):
    ...
    for entry in unique_entries:
      copy = reduced_part[:]
      copy.remove(entry)  # Remove the set of adjacent lines we're splitting by crossing some lines in it ...
      if len(poss_partitions_special_v2(entry,adj_lines)) > 0:  # remember that if nothing is returned, we haven't split a set of adjacent lines 
                                                             # into two - we've actually crossed the whole set off.
        for element in poss_partitions_special_v2(entry,adj_lines):   # each element is a possible new bit of the partition
          new_copy = copy[:]
          new_copy.extend(element)   # now new_copy is our old partition without the set of adjacent lines we split up by crossing
                                     # some off and but with the resulting new bits due to that move
          ...
          if remove_4s_and_1_pairs(new_copy) != []: partitions_list.append(sorted(remove_4s_and_1_pairs(new_copy)))
      else:
        if remove_4s_and_1_pairs(copy) != []: partitions_list.append(remove_4s_and_1_pairs(copy))
